...
class tex(Task.Task):
	texfun_clean,vars_clean=Task.compile_fun('rm -f ${LOF_FILES}',shell=False)
							...
							for tsk in self.generator.tasks:
								...
								for ext in exts_tex:
									if found.name.endswith(ext):
										parse_node(found)
										break
							Logs.debug('tex: trying %s%s (absolute)'%(path,k))
							found=self.generator.path.find_resource(path+k)
							for tsk in self.generator.tasks:
								if not found or found in tsk.outputs:
									break
							else:
...
class dvipdf(Task.Task):
  texfun,vars=Task.compile_fun('${DVIPDF} ${DVIPDFFLAGS} ${SRC} ${TGT}',shell=True)
  color='BLUE'
  after=['latex','pdflatex','xelatex']
  ...
  def run(self):
    ...
    print f
    print self.env
    self.check_status('error when calling latex',f())
...
def apply_tex(self):
	...
	for node in self.to_nodes(self.source):
		...
		v=dict(os.environ)
		print "path: ",node.abspath()
		p=node.parent.abspath()+os.pathsep\
                    +node.parent.get_bld().abspath()+os.pathsep\
		    +v.get('TEXINPUTS','')+os.pathsep\
		    +self.path.get_src().abspath()+os.pathsep\
		    +self.path.get_bld().abspath()+os.pathsep
		    #+v.get('TEXINPUTS','')+os.pathsep
		v['TEXINPUTS']=p
		v['TEXPICTS']=p
		if self.type=='latex':
