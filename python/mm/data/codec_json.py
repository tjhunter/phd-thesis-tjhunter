...
import datetime
from mm.data.structures import Spot, Route, TSpot, RouteTT
...
def decode_RouteTT(dct):
  start_time = decode_time(dct['startTime'])
  end_time = decode_time(dct['endTime'])
  route = decode_Route(dct['routes'][0])
  vehicle_id = dct['id']
  return RouteTT(route, start_time, end_time, vehicle_id)
